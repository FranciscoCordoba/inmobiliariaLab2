...
export function Propietarios() {
  ...
  const validateForm = () => {
    ...
    if (!currentPropietario.nombre.trim()) errors.nombre = "El nombre es obligatorio";
    if (!currentPropietario.apellido.trim()) errors.apellido = "El apellido es obligatorio";
    ...
    if (!currentPropietario.telefono.trim()) errors.telefono = "El teléfono es obligatorio";
    if (!currentPropietario.email.trim()) {
      errors.email = "El correo electrónico es obligatorio";
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(currentPropietario.email)) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Administra altas, bajas y modificaciones de los propietarios de inmuebles.
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-xs font-medium text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg w-full sm:w-auto text-center">
          Total de propietarios: <span className="font-bold text-slate-700">{propietarios.length}</span>
        </div>
      </div>
